...
# Test effect of using prior for p_B when estimating params
...
# calculate old prior, just prior for pB, other priors flat/improper
## truncate normal distribution b/c pB must be in range [0,1]
OldPrior <- dtruncnorm(Oldpb, a=0, b=1, mean=p_b, sd=0.05)
...
filename <- paste("2gpsMC2gpsData_pA",p_a,"_pB",p_b,"_fA",f_a,"_F",Focal,"_N",N,"_N2",N2,"_len",steps,"_sims",sims,"pBpriorseed.csv", sep="")
...
for(i in 1:steps)
{
  ...
  # calculate new prior, just prior for pB, other priors flat/improper
  ## truncate normal distribution b/c pB must be in range [0,1]
  NewPrior <- dtruncnorm(Newpb, a=0, b=1, mean=p_b, sd=0.05)
