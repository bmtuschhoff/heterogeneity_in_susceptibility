# Calculate power to detect heterogeneity in suscepitibility given contact (HiS) in the discrete case (with 2 types) across parameter combinations using simulated contact tracing data and a likelihood ratio test
...
filename <- paste("2gpPowers_fA",f_a_old,"_F",Focal,"_N",N,"_N2",N2,"_sims",num_sims,".csv",sep="")
...
find_power <- function(p_a, p_b)
{
  ...
  for(sim in 1:num_sims)
  {
    ...
    # calculate new fraction of type A after 1st exposure event
  ...
  for(f in (1:Focal))
  {
    ...
    for(j in (1:num_sims))
    {
      if(!is.nan(f_a_new[j]))
      {
        # use unif RV to decide if choose type A or B and check for infection
  ...
  # calculate probabilities of inf across all individuals as best estimates
  p_naive <- naive_inf / naive_pop			# naive individuals
  p_prev <- prev_inf / Focal				# focal individuals
  p_avg <- (naive_inf + prev_inf) / (naive_pop + Focal)	# avg individual
  ...
  return(c(f_inf, cv, power, sd(ratio_tests)/mean(ratio_tests), mean(p_avg), sd(p_avg), mean(p_naive), sd(p_naive), mean(p_prev), sd(p_prev)))
}
