...
# Test effect of N on estimation by subsetting data with bigger N
...
inf2_prev <- unlist(read.csv("inf2_prev_k0.591715976331361_theta0.626097060979711_F5000_N100_N2100_len6e+05_sims600seed.csv", row.names=1), use.names=F)
inf2_naive_old <- unlist(read.csv("inf2_naive_k0.591715976331361_theta0.626097060979711_F5000_N100_N2100_len6e+05_sims600seed.csv", row.names=1), use.names=F)

set.seed(3)

##############################################################################
### subset naive indivs to N=5

# draw number of infected naive indivs for smaller N
# out of N-1 indivs that can possibly be infected
# prob of being infected based on how many infected with bigger N out of number that could be inf there 
inf2_naive <- rbinom(Focal, N-1, inf2_naive_old/(oldN-1))
...
fileName <- paste("ContMCContdata_k",k,"_theta",theta,"_F",Focal,"_N",N,"_N2",N2,"_len",steps,"_sims",sims,"N100subseed.csv",sep="")
