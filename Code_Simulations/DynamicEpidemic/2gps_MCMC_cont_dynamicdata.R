# Estimate parameters for heterogeneity in suscepitibility given contact (HiS) in the discrete case (with 2 types) using simulated contact tracing data (dynam_epi_model_HiST_contactTracing_discrete.cpp) and Metropolis-Hastings MCMC with ABC
...
contact_data <- read.csv("contact_data_dynamepi_pA0.74809046024617_pB0.125477384938458_c1.3e0.25fA0.2_F5000.csv", row.names=1)
inf2_prev <- sum(contact_data$Focal_infected)
inf2_naive <- sum(contact_data$Naive_infected)
exp2_prev <- sum(contact_data$Focal_exposed)
exp2_naive <- sum(contact_data$Naive_exposed)
...
Log_Like <- function(pa_val, pb_val, fa_val, Focal, sims, inf2_prev, inf2_naive, exp2_prev, exp2_naive)
{
  ...
  LFocal <- sum(abs(TotFocalInf - inf2_prev) / inf2_prev <= 0.01) / sims
  LNonFocal <- sum(abs(TotNonFocalInf - inf2_naive) / inf2_naive <= 0.01) / sims
...
while(OldL == -Inf)
{
  ...
  OldL <- Log_Like(Oldpa, Oldpb, Oldfa, Focal, sims, inf2_prev, inf2_naive, exp2_prev, exp2_naive)
}
...
filename <- paste("2gpsMC2gpsDataDynamEpi_pA",p_a,"_pB",p_b,"_fA",f_a,"_F",Focal,"_len",steps,"_sims",sims,"tol0.013seed.csv", sep="")
...
for(i in 1:steps)
{
  ...
  NewL <- Log_Like(Newpa, Newpb, Newfa, Focal, sims, inf2_prev, inf2_naive, exp2_prev, exp2_naive)
