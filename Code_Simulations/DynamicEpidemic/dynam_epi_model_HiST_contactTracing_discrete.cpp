// individual based SIR Gillespie model including different forms of heterogeneity, discrete (2 groups) case for HiS
...
// note: must find risk of type B individual being infected from 2gps_FindrB.R and read in
...
double p_A, p_B;		// probabilities of infection for type A and B individuals
double r_A = -log(1.0-0.74809046024617), r_B = -log(1.0-0.125477384938458);		// risks of infection for type A and B individuals
double f_A = 0.2;		// fraction of initial population that is type A
...
int main(int argc, char *argv[])
{
	...
	double m, psi;	// shape params for HiT, HiC distributions
	...
	int is_A;	// check if individual is type A or not for assigning infection probability
	...
	{
		// back calculate for r_A, p_A, p_B
		r_A = -log( 1.0 - (1.0/f_A) * (f_inf - (1.0 - exp(-r_B))*(1.0 - f_A)) );
		p_A = 1.0 - exp(-r_A);
		p_B = 1.0 - exp(-r_B);
	}
	...
	prob_inf_avg = p_A*f_A + p_B*(1.0 - f_A);
	...
	for(indiv = 0; indiv < pop_size; indiv++)
	{
		...
		{
			is_A = gsl_ran_binomial(rng, f_A, 1);	// check if indiv is type A, 1=A and 0=B
			if(is_A == 1)	// indiv is type A
				risk_Snaive[indiv] = r_A;
			else		// indiv is type B
				risk_Snaive[indiv] = r_B;
		}
		else	// no HiS
...
void expose(vector<double> &risk_Snaive, vector<double> &lamb_Snaive, vector<double> &c_rate_Snaive, vector<int> &index_Snaive, vector<double> &risk_Sprev, vector<double> &lamb_Sprev, vector<double> &c_rate_Sprev, vector<int> &index_Sprev, vector<double> &risk_I, vector<double> &lamb_I, vector<double> &c_rate_I, vector<int> &index_I, vector<int> &expose_naive, vector<int> &inf_naive, vector<int> &expose_prev, vector<int> &inf_prev, double beta_avg, int *numS_naive, int *numS_prev, int *numI, gsl_rng *rng, mt19937 &rng2)
{
	...
	if(S_expose < *numS_naive)	// S indiv is naive
	{
		...
		is_inf = gsl_ran_binomial(rng, 1.0-exp(-infect_rate), 1);		// use bernoulli to check if indiv infected
...
// find contact rate between individuals
double c_rate(int S_index, int I_index)
{
	return(1.0);
}
...
bool parseOptions(int argc, char *argv[])
{
	for(int a = 1; a < argc; a++)
	{
		...
		if(strcmp(argv[a], "-cs") == 0 && a + 1 < argc)	// set cv for HiS
			cv_s = atof(argv[++a]);
		if(strcmp(argv[a], "-ra") == 0 && a + 1 < argc)	// set risk for type A individuals
			r_A = atof(argv[++a]);
		if(strcmp(argv[a], "-rb") == 0 && a + 1 < argc)	// set risk for type B individuals
			r_B = atof(argv[++a]);
		if(strcmp(argv[a], "-fa") == 0 && a + 1 < argc) // set fraction of initial population that is type A
			f_A = atof(argv[++a]);
		if(strcmp(argv[a], "-ct") == 0 && a + 1 < argc)	// set cv for HiT
			cv_t = atof(argv[++a]);
		if(strcmp(argv[a], "-f") == 0 && a + 1 < argc)	// set expected fraction of individuals infected, to set scale theta for HiS
			f_inf = atof(argv[++a]);
